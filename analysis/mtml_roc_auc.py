...
"""
MTML ROC-AUC Comparison (Reptile-ST, Reptile-MT, Reptile-MI)
Loads global_roc_data.pkl from each of the three MTML methods and
produces combined ROC curves, a grouped bar chart, and an AUC summary CSV.
...
def parse_args():
    p = argparse.ArgumentParser(description='MTML ROC-AUC comparison (ST, MT, MI)')
    p.add_argument('--dataset', type=str, default='vreed',
                   choices=['vreed', 'dssn_eq', 'dssn_em'],
                   help='Dataset to analyse (default: vreed)')
    return p.parse_args()
...
# Method display names → subdirectory suffixes (appended to {prefix}_MTML/{prefix}_)
METHOD_SUBDIRS = {
    'Reptile-ST': 'reptile_st',
    'Reptile-MT': 'reptile_mt',
    'Reptile-MI': 'reptile_mi',
}
...
def plot_auc_bar(ar_results, va_results, save_path, prefix):
    ...
    ax.set_xlabel('Method', fontsize=14, fontweight='bold')
    ax.set_ylabel('AUC Score', fontsize=14, fontweight='bold')
    ax.set_title(f'AUC Comparison — MTML Methods ({prefix})', fontsize=16,
                 fontweight='bold', pad=20)
    ax.set_xticks(x)
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(RESULTS_DIR, f'{prefix}_MTML', 'ROC_Comparisons_MTML')
    os.makedirs(output_dir, exist_ok=True)
    ...
    ar_results = plot_roc_comparison(
        all_roc_data, 'AR',
        os.path.join(output_dir, 'ROC_Comparison_AR_MTML.png'),
        prefix)
    ...
    va_results = plot_roc_comparison(
        all_roc_data, 'VA',
        os.path.join(output_dir, 'ROC_Comparison_VA_MTML.png'),
        prefix)
    ...
    print('=' * 70)
    plot_auc_bar(ar_results, va_results,
                 os.path.join(output_dir, 'AUC_Bar_Comparison_MTML.png'),
                 prefix)
    ...
    summary_df = pd.DataFrame(rows).rename(
        columns={'method': 'Method', 'auc': 'AUC', 'n_samples': 'N_Samples'})
    csv_path = os.path.join(output_dir, 'ROC_AUC_Summary_MTML.csv')
    summary_df.to_csv(csv_path, index=False)
    print(f'\n✓ Saved: {csv_path}')
