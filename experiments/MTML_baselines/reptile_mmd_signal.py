"""
Reptile MMD-Signal — anchor-similar-divergent episode sampling guided by
MMD² on raw per-channel signal values.

For each pair of participants (u, v) and each physiological channel c,
MMD²(X_u^{(c)}, X_v^{(c)}) is computed on raw 1-D signal samples with a
Gaussian kernel (median-heuristic bandwidth). Per-channel MMD² values are
averaged across channels. Similarity = -MMD² so that highest = most
similar, matching the anchor-high/low convention from reptile_mi.
...
# Reuse the episode sampler and inner-loop adaptation from reptile_mi.
# (sample_mi_guided_episode operates on any similarity matrix where
# higher = more similar — it does not assume the values come from MI.)
...
def parse_args():
    p = argparse.ArgumentParser(description='Reptile MMD-Signal')
...
# ================================================================
# MMD-GUIDED TASK EPISODES (per-channel raw signal values)
# ================================================================

def build_task_mmd_signal_matrix(tasks_data, splits, feature_cols=None,
                                  max_samples_per_channel=10000,
                                  use_train_trials_only=True, seed=42):
    """
    Pairwise similarity = -MMD² between participants on raw per-channel
    signal values, averaged across channels. Each channel is treated as
    a 1-D sample for the Gaussian-kernel MMD estimator.
    """
    ...
    # Subsample per-channel raw values per participant (kernel cost is
    # O(N^2) per channel per pair, so cap N for tractability).
    signals = {}
    for pid in task_ids:
        ...
        for col in feature_cols:
            x = task_df[col].values.astype(np.float32)
            if len(x) > max_samples_per_channel:
                idx = rng.choice(len(x), max_samples_per_channel, replace=False)
                x = x[idx]
            per_channel.append(x.reshape(-1, 1))   # (N, 1) for kernel
        signals[pid] = per_channel
    ...
    for i, pid_i in enumerate(task_ids):
        for j, pid_j in enumerate(task_ids):
            ...
            for c in range(len(feature_cols)):
                s = mmd_similarity(signals[pid_i][c], signals[pid_j][c],
                                   rng=rng)
                sims.append(s)
            sim = float(np.mean(sims))
            sim_matrix[pid_i][pid_j] = sim
            sim_matrix[pid_j][pid_i] = sim
...
def _reptile_train(label_type, df, splits, train_ps, cfg, device, output_dir,
    ...
    """Reptile-MMD-Signal: MMD-guided episodes on raw per-channel signals."""
    ...
    tasks_data = {uid: df[df['ID'] == uid].reset_index(drop=True)
                  for uid in train_ps}
    sim_matrix = build_task_mmd_signal_matrix(
        tasks_data, splits,
    ...
    print(f"\n[FINAL-{label_type.upper()}] Meta-training on "
          f"{len(task_ids)} train participants (MMD-Signal)")
    ...
    for step in range(meta_steps):
        ...
        if (step + 1) % 10 == 0 or step == 0:
            print(f"  [{label_type.upper()}] Reptile-MMD-Signal "
                  f"step {step+1}/{meta_steps}")

    torch.save(base.state_dict(),
               os.path.join(output_dir,
                            f'reptile_mmd_signal_base_{label_type}.pth'))
    return base
...
if __name__ == '__main__':
    ...
    for lt in ['ar', 'va']:
        h = hp[lt]
        print(f"\n{'='*60}\nREPTILE-MMD-SIGNAL META-TRAINING "
              f"{lt.upper()}\n{'='*60}")
        set_all_seeds(SEED)
        base = _reptile_train(
    ...
    with open(os.path.join(output_dir,
                           'reptile_mmd_signal_results.pkl'), 'wb') as f:
        pickle.dump(final, f)
