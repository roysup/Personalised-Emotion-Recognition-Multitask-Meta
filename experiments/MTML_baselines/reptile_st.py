...
Shared backbone + single shared head.
Reptile outer-loop averages adapted weights across episode participants.
...
def _reptile_train(label_type, df, splits, train_ps, cfg, device, output_dir):
    ...
    torch.save(base.state_dict(), os.path.join(output_dir, f'reptile_st_base_{label_type}.pth'))
    torch.save(head.state_dict(), os.path.join(output_dir, f'reptile_st_head_{label_type}.pth'))
    return base, head
