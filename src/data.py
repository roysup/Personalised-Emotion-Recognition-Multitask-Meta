...
# =============================
# BALANCED K-SHOT SUBSAMPLING
# =============================

def _balanced_subsample_indices(y_target, k_per_class, rng,
                                fallback_cap=None):
    """
    Select k samples per class for balanced k-shot support sets.

    Parameters
    ----------
    y_target     : 1D array of binary labels (0/1)
    k_per_class  : int — number of samples to select per class
    rng          : numpy Generator for reproducibility
    fallback_cap : int or None — if a class is missing, cap the uniform
                   fallback at this many total samples (default: 2*k_per_class)

    Returns
    -------
    idx : sorted array of selected indices
    """
    y = y_target.astype(int).reshape(-1)
    idx0 = np.flatnonzero(y == 0)
    idx1 = np.flatnonzero(y == 1)

    # Fallback: if one class is entirely absent, sample uniformly up to cap
    if len(idx0) == 0 or len(idx1) == 0:
        cap = fallback_cap if fallback_cap is not None else 2 * k_per_class
        n = min(cap, len(y))
        return rng.choice(len(y), size=n, replace=False)

    # Take min(k_per_class, available) per class
    k_eff = min(k_per_class, len(idx0), len(idx1))
    s0 = rng.choice(idx0, size=k_eff, replace=False)
    s1 = rng.choice(idx1, size=k_eff, replace=False)
    return np.sort(np.concatenate([s0, s1]))
...
def build_support_query(task_df, support_trials, query_trials, ar_or_va='ar',
    ...
    # Balanced k-shot subsampling on the support set
    if balanced_k_per_class is not None and len(Xs) > 0:
        y_target = yas if ar_or_va == 'ar' else yvs
        idx = _balanced_subsample_indices(
            y_target, balanced_k_per_class, rng,
            fallback_cap=2 * balanced_k_per_class)
        Xs  = Xs[idx]
        yas = yas[idx]
        yvs = yvs[idx]
